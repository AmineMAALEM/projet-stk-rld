...
# Imports locaux (Try/Except pour compatibilité test local)
try:
    from .actors import Actor
    from .wrappers import ProcessingWrapper, ActionConversionWrapper
except ImportError:
    from actors import Actor
    from wrappers import ProcessingWrapper, ActionConversionWrapper
...
def get_wrappers() -> List[Callable[[gym.Env], gym.Wrapper]]:
    # ORDRE D'EXECUTION (Intérieur -> Extérieur pour les Observations)
    return [
        # 1. Standardise la taille des tableaux (5 items, 5 karts, 5 chemins)
        lambda env: ConstantSizedObservations(env, state_items=5, state_karts=5, state_paths=5),
        
        # 2. Convertit en coordonnées Polaires (Essentiel pour ton modèle)
        lambda env: PolarObservations(env),
        
        # 3. Ton wrapper personnalisé (Sanitize -> Features -> Flatten -> Stack)
        lambda env: ProcessingWrapper(env, n_stack=4),
        
        # 4. Conversion Action (Extérieur)
